...
export function useMyOrganization(enabled = true) {
  return useQuery({
    ...
    staleTime: 10 * 60 * 1000, // cache 10 min — org data rarely changes
    enabled: !!enabled,
  });
}
...
import { listAssets, getAssetSummary, getAssetCategories, getMyAssets } from "@/lib/api";
...
export function useAssets(params = {}) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return res.ok ? res.data : { assets: [], total: 0, pages: 1 };
    },
    keepPreviousData: true,
  });
}
...
export function useAssetCategories() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return res.ok ? (res.data.categories || res.data || []) : [];
    },
  });
}
...
export function useMyAssets() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return res.ok ? res.data : { assets: [], total: 0 };
    },
  });
}


// ── Exit Management Queries ───────────────────────────────────────────────

import { listExitRequests, getMyExitStatus, getExitReportSummary } from "@/lib/api";

export function useExitRequests(params = {}) {
  return useQuery({
    queryKey: ["exit-requests", params],
    queryFn: async () => {
      const res = await listExitRequests(params);
      return res.ok ? res.data : { exits: [], total: 0, pages: 1 };
    },
    keepPreviousData: true,
  });
}

export function useMyExitStatus() {
  return useQuery({
    queryKey: ["my-exit"],
    queryFn: async () => {
...
export function useExitReportSummary() {
  return useQuery({
    queryKey: ["exit-summary"],
    queryFn: async () => {
      const res = await getExitReportSummary();
      return res.ok ? res.data : null;
    },
  });
}
...
import { listTeams, getTeamDetail } from "@/lib/api";
...
export function useTeams(params = {}) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return res.ok ? res.data : { teams: [], total: 0 };
    },
  });
}
