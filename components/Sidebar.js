...
export default function Sidebar() {
  ...
  const getRoleBadge = () => {
    switch (user.role) {
      ...
      case "orgadmin":
        return { label: "Org Admin", bg: "bg-indigo-50 border border-indigo-100", text: "text-indigo-600" };
      case "hr":
        return { label: "HR Manager", bg: "bg-blue-50 border border-blue-100", text: "text-blue-600" };
      case "employee":
                ...
                >
                    ...
                    {!collapsed && (
                      <motion.span
                        ...
                        className={`text-sm font-semibold whitespace-nowrap ${
                          isActive ? (isSuperAdmin ? "text-amber-700" : "text-brand-700") : ""
                        }`}
                      >
                        {item.label}
                      </motion.span>
                  ...
                  {isActive && !collapsed && (
                    <motion.div
                      ...
                      className={`ml-auto w-1.5 h-1.5 rounded-full ${isSuperAdmin ? "bg-amber-500" : "bg-brand-500"}`}
                    />
                  )}
                </motion.div>
              </Link>
