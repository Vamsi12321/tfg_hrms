...
export default function MyWellnessPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (res.ok) {
      ...
      if (msg.toLowerCase().includes("already")) {
        setSubmitted(true);
        showToast("You've already logged your mood today! 🎉", "success");
      } else {
        showToast(msg, "error");
      }
    }
  ...
  if (loading) return <div className="min-h-screen bg-surface-100"><TopBar title="Wellness" /><div className="p-6 flex items-center justify-center"><div className="w-8 h-8 border-2 border-brand-200 border-t-brand-600 rounded-full animate-spin" /></div></div>;
