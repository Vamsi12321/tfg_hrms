...
export default function HRPayrollPage() {
  ...
  // Forms
  const [configForm, setConfigForm] = useState({ pf_percentage:12, esi_employee_percentage:0.75, esi_employer_percentage:3.25, esi_limit:21000, professional_tax:200, pay_day:28, lop_calculation:"calendar_days", basic_percentage:40, hra_percentage:20, special_allowance_percentage:15 });
      ...
      <div className="p-6 space-y-6">
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                <table className="w-full">
                  <thead><tr className="bg-slate-50/80">
                    {["Employee","Department","Gross","Deductions","Net Pay","Status",""].map(h=><th key={h} className="text-left text-[10px] font-bold text-slate-500 uppercase px-4 py-3">{h}</th>)}
                  </tr></thead>
                  <tbody>
                    {payslips.map((ps, i) => {
                      ...
                      return (
                        <tr key={ps.id||i} className="border-t border-slate-50 hover:bg-slate-50/50">
                          ...
                          <td className="px-4 py-3 text-xs text-slate-600">{ps.department}</td>
                          <td className="px-4 py-3 text-xs font-bold text-slate-700">{fmt(ps.gross_pay)}</td>
                          <td className="px-4 py-3 text-xs text-red-500">{fmt(ps.total_deductions)}</td>
                          <td className="px-4 py-3 text-sm font-black text-green-600">{fmt(ps.net_pay)}</td>
                          ...
                          <td className="px-4 py-3">
                            {(ps.status === "processed" || ps.status === "draft") && (
                              <button onClick={()=>{setShowEditPayslip(ps);setEditForm({bonus:ps.earnings?.bonus||0,reimbursements:ps.earnings?.reimbursements||0,tds:ps.deductions?.tds||0,other_deductions:ps.deductions?.other_deductions||0});}}
                                className="text-[10px] font-bold text-brand-600 hover:underline flex items-center gap-1"><Edit className="w-3 h-3" /> Edit</button>
                            )}
                          </td>
                        </tr>
                      );
        ...
        {tab === "config" && config && (
          <motion.div initial={{ opacity:0, y:15 }} animate={{ opacity:1, y:0 }}
            ...
            <p className="text-[10px] text-slate-400 mt-3">LOP Calculation: {config.lop_calculation || "calendar_days"}</p>
