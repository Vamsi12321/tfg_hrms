  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-900">Employee Payslips</h2>
          <p className="text-sm text-slate-500">View and manage monthly payslips</p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-3 py-2.5 flex-1 min-w-[200px] max-w-xs focus-within:border-brand-400">
      ...
      {summary&&(
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: "Total Employees", value: summary.total_employees||summary.employee_count||payslips.length, color: "text-indigo-700", bg: "bg-indigo-50/60", border: "border-indigo-100/40", dot: "bg-indigo-500" },
            { label: "Gross Payroll", value: fmt(summary.total_gross||summary.gross), color: "text-emerald-700", bg: "bg-emerald-50/60", border: "border-emerald-100/40", dot: "bg-emerald-500" },
            { label: "Total Deductions", value: fmt(summary.total_deductions||summary.deductions), color: "text-rose-700", bg: "bg-rose-50/60", border: "border-rose-100/40", dot: "bg-rose-500" },
            { label: "Net Payroll", value: fmt(summary.total_net||summary.net), color: "text-brand-700", bg: "bg-brand-50/60", border: "border-brand-100/40", dot: "bg-brand-500" }
          ].map((k) => (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead><tr className="bg-gradient-to-r from-indigo-50 via-slate-50 to-blue-50/60">
                {["Employee","Department","Days","LOP","Gross","Deductions","Net Pay","Status","Actions"].map(h=><th key={h} className="text-left text-[10px] font-bold text-indigo-700 uppercase tracking-wider px-4 py-4 whitespace-nowrap">{h}</th>)}
              </tr></thead>
              <tbody className="divide-y divide-slate-50">
                {filtered.map((ps,i)=>{
                  const sc=statusCfg[ps.status]||statusCfg.draft;
                  return (
                    <motion.tr key={ps.id||i} initial={{opacity:0}} animate={{opacity:1}} transition={{delay:i*0.02}} className="hover:bg-indigo-50/20 transition-colors group">
                      <td className="px-4 py-4">
                        <p className="text-sm font-bold text-slate-900">{ps.employee_name}</p>
                        <p className="text-[10px] text-slate-400 font-mono">{ps.employee_code}</p>
                      </td>
                      <td className="px-4 py-4">
                        <span className="text-xs font-semibold text-slate-600 bg-slate-100 px-2.5 py-1 rounded-full">{ps.department||"—"}</span>
                      </td>
                      <td className="px-4 py-4">
                        <span className="text-xs font-bold text-emerald-700 bg-emerald-50 px-2 py-1 rounded-lg">{ps.days_worked}</span>
                        <span className="text-xs text-slate-400">/{ps.working_days}</span>
                      </td>
                      <td className="px-4 py-4">
                        <span className={`text-xs font-bold px-2 py-1 rounded-lg ${(ps.lop_days||0)>0?"text-red-600 bg-red-50":"text-slate-400 bg-slate-50"}`}>{ps.lop_days||0}</span>
                      </td>
                      <td className="px-4 py-4 text-sm font-bold text-slate-700">{fmt(ps.gross_salary||ps.gross_pay)}</td>
                      <td className="px-4 py-4 text-xs font-semibold text-rose-600">-{fmt(ps.total_deductions)}</td>
                      <td className="px-4 py-4 text-base font-black text-emerald-600">{fmt(ps.net_pay)}</td>
                      <td className="px-4 py-4"><span className={`text-[9px] font-bold px-2.5 py-1 rounded-full border ${sc.cls}`}>{sc.label}</span></td>
                      <td className="px-4 py-4">
                        <div className="flex gap-1.5">
                          <button onClick={()=>setShowDetail(ps)} className="w-7 h-7 rounded-lg bg-brand-50 hover:bg-brand-100 flex items-center justify-center transition-colors" title="View"><Eye className="w-3.5 h-3.5 text-brand-600"/></button>
                          {(ps.status==="processed"||ps.status==="draft")&&<button onClick={()=>{setShowEdit(ps);setEditForm({bonus:ps.earnings?.bonus||0,reimbursements:ps.earnings?.reimbursements||0,tds:ps.deductions?.tds||0,other_deductions:ps.deductions?.other_deductions||0});}} className="w-7 h-7 rounded-lg bg-indigo-50 hover:bg-indigo-100 flex items-center justify-center transition-colors" title="Edit"><Edit className="w-3.5 h-3.5 text-indigo-600"/></button>}
                        </div>
                      </td>
                    </motion.tr>
                  );
