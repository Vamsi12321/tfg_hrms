...
import {
  Search, Calendar, Filter, RefreshCw, Activity,
  Users, Clock, Shield, FileText, Bell, Heart, Target,
...
export default function OrgLogsPage() {
  ...
  const [filters, setFilters] = useState({ module:"", action:"", user_role:"", from_date:"", to_date:"", search:"" });
      ...
      <div className="p-6 space-y-6">
        ...
        {stats && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <motion.div initial={{ opacity:0,y:15 }} animate={{ opacity:1,y:0 }}
              ...
              <p className="text-2xl font-black text-brand-600">{stats.total_today}</p>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <select value={filters.user_role} onChange={e => { setFilters(f=>({...f,user_role:e.target.value})); setPage(1); }}
            ...
            {["org_admin","hr_admin","employee"].map(r => <option key={r} value={r}>{r}</option>)}
          </select>
                  ...
                  <motion.div key={log.id || i} initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:i*0.02 }}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1 flex-wrap">
                        <span className="text-[10px] text-slate-500 font-medium">{log.user_name}</span>
