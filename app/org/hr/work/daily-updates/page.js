...
import { MessageSquare, AlertCircle, CheckCircle2, Calendar, AlertTriangle } from "lucide-react";
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <p className="text-xs text-slate-500">{updates.length} update{updates.length!==1?"s":""} for {new Date(date+"T00:00:00").toLocaleDateString("en-IN",{weekday:"long",day:"numeric",month:"short"})}</p>
      </div>
      ...
      : updates.length===0 ? (
        <div className="bg-white rounded-2xl p-12 border border-slate-100 shadow-sm text-center">
          ...
          <p className="text-sm font-semibold text-slate-400">No daily updates submitted for {date}</p>
            ...
            return (
              <motion.div key={u.id||i} initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{delay:i*0.04}}
                ...
                <div className="flex items-start gap-4">
                  {/* Avatar */}
                  <div className="w-10 h-10 rounded-xl bg-brand-100 flex items-center justify-center text-sm font-bold text-brand-700 flex-shrink-0">
                    {initials}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-2">
                      <h4 className="text-sm font-bold text-slate-900">{u.employee_name}</h4>
                      {u.department && <span className="text-[9px] font-semibold text-slate-400 bg-slate-50 px-2 py-0.5 rounded-full">{u.department}</span>}
                    ...
                    <div className="space-y-2.5">
                      ...
                      {u.yesterday && (
                        <div className="flex items-start gap-2.5">
                          ...
                          <div>
                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Yesterday</p>
                            <p className="text-xs text-slate-700 leading-relaxed mt-0.5">{u.yesterday}</p>
                          </div>
                        </div>
                      )}
                      ...
                      {u.today && (
                        <div className="flex items-start gap-2.5">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-700 leading-relaxed mt-0.5">{u.today}</p>
                          </div>
                        </div>
                      )}
                      ...
                      {hasBlocker && (
                        <div className="flex items-start gap-2.5">
                          ...
                          <div>
                            <p className="text-[9px] font-bold text-amber-600 uppercase tracking-wider">Blocker</p>
                            <p className="text-xs text-amber-700 leading-relaxed mt-0.5 font-medium">{u.blockers}</p>
                          </div>
                        </div>
                      )}
                    </div>
